...
select year(order_purchase_timestamp), count(order_id) from orders 
...
select p.product_category, round(sum(o.price), 2) as total_sales from order_items as o
join products as p
...
SELECT 
    ROUND(COUNT(DISTINCT CASE
            ...
            2) AS Percentage_installments_orders
FROM
    payments;
...
select seller_id, total_sales, rank() over (order by total_sales desc) as ranks
from (
